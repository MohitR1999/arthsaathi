    ...
    if (CashFlowCategory) {
        const post = async (request: Request, response: Response) => {
            ...
            if (request.headers.userId && type) {
                ...
                response.status(STATUS_CODES.CREATED).json({ message: MESSAGES.SUB_CATEGORY_CREATED });
            } else {
                response.status(STATUS_CODES.UNAUTHORIZED).json({ message: MESSAGES.UNAUTHORIZED });
            }
        };
        return post;
    } 
    return (request: Request, response: Response) => {
        response.status(STATUS_CODES.SERVER_ERROR).json({ message: 'Method not working!' });
    };
